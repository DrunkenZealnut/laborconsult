#!/usr/bin/env python3
"""
노동OK BEST Q&A 2025년 게시글 → Pinecone 벡터 업로드  ⛔ **실행 봉인됨 (2026-08-23)**

봉인 사유(`pinecone_upload.py`와 같은 계열 결함):
1. **전용 인덱스가 실재하지 않는다** — 기본값 `laborconsult-bestqna-2025`는 계정에
   없다(실측 2026-08-23: 계정 인덱스는 semiconductor-lithography / accmanage /
   safecam / osh-rag / laborsafecam 5개뿐). 실행하면 `get_or_create_index`가 **새
   인덱스를 만들고**, 프로덕션 검색(`app/core/rag.py::NS_GROUPS`)은 그 인덱스를
   읽지 않으므로 임베딩 비용만 쓰고 검색에는 영원히 반영되지 않는다.
2. **`--reset`이 인덱스를 통째로 삭제했다** — `pc.delete_index()`라 네임스페이스가
   아니라 인덱스 전체가 대상이다. 이 계정에는 타 프로젝트 인덱스가 4개 더 있고
   대상 인덱스명은 `PINECONE_INDEX_NAME_2025` 환경변수로 바뀌므로, **환경변수 하나로
...
def load_metadata() -> dict:
    if not os.path.exists(METADATA_FILE):
        print("metadata_2025.json이 없습니다. generate_metadata_2025.py를 먼저 실행합니다...\n")
        import subprocess
        subprocess.run(
            [sys.executable, os.path.join(BASE_DIR, "generate_metadata_2025.py")],
            check=True,
        )
