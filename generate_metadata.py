#!/usr/bin/env python3
"""
output/ 폴더의 markdown 파일들을 분석하여 metadata.json 생성

생성되는 metadata.json은:
  1. pinecone_upload.py의 업로드 소스
...
def parse_md(filepath: str) -> dict:
    ...
    # 날짜를 정렬용 숫자로 변환 (2022.12.28 → 20221228)
    date_num = 0
    if date:
        try:
    ...
    return {
        ...
        "title": title,
        "date": date,
        "date_num": date_num,       # 날짜 숫자 (range 필터용)
        "views": views,
        "url": url,
        "filename": filename,
        "char_count": char_count,
        "sections": sections,       # 게시글 내 섹션 목록
        "chunk_count": 0,           # pinecone_upload.py 실행 후 갱신
