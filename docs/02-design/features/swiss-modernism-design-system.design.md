...
| D-3 | `body max-width` | **이중 컨테이너** | 읽기 영역 800px(`--reading-max`), 구조 영역 1280px(`--container-max`). 채팅 답변은 행 길이가 가독성을 좌우하므로 1280px로 넓히지 않는다. **구조 영역의 적용 대상은 §8.7에서 관리자 대시보드로 한정했다.** |
...
| 토큰 | 값 | 용도 |
|------|-----|------|
| `--leading-none` | `1` | 버튼 등 단일 행 |
| `--leading-tight` | `1.2` | 큰 제목 |
| `--leading-snug` | `1.4` | 소제목·레이블 |
| `--leading-base` | `1.5` | 조밀한 본문 |
| `--leading-normal` | `1.6` | 본문 기본 |
| `--leading-relaxed` | `1.75` | 긴 글(약관·정책) |
| `--weight-normal` | `400` | 본문 |
| `--weight-medium` | `500` | 레이블·버튼 |
| `--weight-semibold` | `600` | 강조 (§8.10 기록된 추가 단계) |
| `--weight-bold` | `700` | 제목 |
...
**선 색만 정의돼 있다. 면 색은 추가해야 한다.** `tokens.css`의 `--chart-1`~`--chart-6`은 **선·솔리드 값 하나씩**만 담는다(관리자 도넛처럼 면을 통째로 칠하는 용도, §8.8). 그러나 흐름도 노드는 `rect`의 **면(fill)과 테두리(stroke)가 서로 다른 색**이므로 토큰 하나로 감당할 수 없다 — 예컨대 `c-blue`는 면 `#E6F1FB`, 테두리 `#185FA5`다.

따라서 흐름도 적용 시 **`--chart-N-fill` 짝을 함께 정의한다.** 값은 아래 매핑표의 fill 열이 그대로 대응한다.

| 역할 | 토큰 | 쓰이는 곳 |
|------|------|-----------|
| 선·솔리드 | `--chart-N` | `rect{stroke}`, `.ts{fill}`(부제), 도넛 조각 |
| 면 | `--chart-N-fill` | `rect{fill}` |
| 제목 | 파생 없음 — `.th{fill}`은 별도 값이라 매핑표에서 직접 가져온다 | `.th{fill}` |

면 색을 선 색에서 **자동 유도하지 않는다.** 명도만 올리는 식으로 계산하면 다크 모드에서 관계가 뒤집혀(다크는 면이 어둡고 선이 밝다) 규칙이 성립하지 않는다. 두 값을 각각 명시한다.
...
| `c-amber` | `--chart-3` | 주의 | `#FAEEDA` / `#804E10` | `#412402` / `#EFC480` |
...
각 클래스는 `rect{fill,stroke}` + `.th{fill}`(제목) + `.ts{fill}`(부제) 3규칙 × 라이트/다크 2모드로 정의된다. stroke 값은 **채도 80% 상한과 배경 대비를 이미 검증했다**(전 색 채도 ≤78%, 대비 ≥6.2:1). amber는 원래 라이트 85%·다크 93%로 상한을 넘어 `#804E10`/`#EFC480`으로 낮췄다. fill 값은 흐름도 적용 시 재검증한다.
...
### 8.6 스페이싱 8px 격자 — 부분 적용

**규격**: 여백·패딩·간격은 8px 배수 토큰만 쓴다(§3.3).
**현재 상태**: 8px 정확 배수 148건은 토큰화했고, 비배수 286건은 리터럴로 남아 있다(이행률 34%).

**이연 근거**: 정확 배수 치환은 값이 같으므로 시각 변화가 0이다. 반면 `10px` `12px` `14px` 같은 비배수를 격자에 맞추려면 **반올림해야 하며 레이아웃이 실제로 움직인다.** `padding: 12px`는 8px로 줄이든 16px로 늘리든 어느 쪽도 무해한 치환이 아니다. 브라우저 실측으로 현재 상태의 무결성(대비 위반 0, 가로 오버플로 0)은 확인했으나, 286건을 일괄 반올림한 뒤의 결과는 화면마다 다시 확인해야 안전하다.
...
**완료 조건** — 다음을 모두 만족해야 이 예외를 닫는다.

1. 비배수 값마다 인접 토큰(8px 배수)으로의 매핑을 확정한다. 값이 두 토큰의 중간이면(예: `12px`) 용도별로 판단한다 — 조밀한 컨트롤은 낮은 쪽, 구획 여백은 높은 쪽.
2. 파일 단위로 적용하고, 각 파일마다 브라우저 실측을 다시 돌린다(라이트/다크 대비 위반 0, 가로 오버플로 0, 모바일 375px 단일 칼럼 붕괴 정상).
3. CI 3종(`test_public_fetch`·`test_answer_renderer`·`test_answer_glance`) 통과.
4. 토큰이 없는 축을 만나면 **먼저 §3.2·§3.3에 토큰을 추가한 뒤** 치환한다. 토큰 없이 리터럴만 옮기면 드리프트가 자리만 바꾼다.
   (`line-height`·`font-weight`가 이 경우였고 `--leading-*`·`--weight-*`로 해소했다.)

   **아직 토큰이 없는 축** — 실측 기준. 스페이싱 작업 전에 함께 정리한다.

   | 축 | 고유 값 | 출현 | 비고 |
   |----|--------:|-----:|------|
   | `z-index` | 8종 (4·50·90·100·200·250·300·400) | 15 | 층위 이름이 없어 새 레이어를 얹을 때 충돌 위험이 가장 크다 |
   | `opacity` | 6종 (0.3~0.9) | 10 | 비활성·오버레이·보조 표시가 뒤섞여 있다 |
   | `blur` | 2종 (4px·16px) | 3 | backdrop 용도 |

### 8.7 구조 컨테이너(1280px) — 관리자 대시보드 한정

**규격**: D-3은 랜딩 히어로·계산기 메뉴·관리자 대시보드 세 영역을 1280px 구조 영역으로 지정했다.
**결정**: **관리자 대시보드에만 적용한다.** 나머지 둘은 대상에서 제외한다.

| 영역 | 적용 | 근거 |
|------|:----:|------|
| 관리자 대시보드 | **적용** | 통계 카드 `span 3`, 차트 `8/4` 비대칭. 다칼럼 데이터 표시라 넓은 폭이 실제로 유용하다 |
| 랜딩(`index`) | 제외 | 중앙 로고 + 채팅 입력 구조다. 1280px로 넓히면 입력창이 과도하게 길어져 오히려 읽기 어려워진다 |
| 계산기 메뉴 | 제외 | `100dvh` 사이드바+뷰어 앱 셸이라 "최대 폭" 개념이 성립하지 않는다 |

설계의 일괄 규칙이 실제 UI와 맞지 않은 사례다. 규칙을 코드에 억지로 맞추는 대신 규칙 쪽을 좁혔다.

**재검토 조건**: 랜딩에 다칼럼 콘텐츠(예: 카테고리 그리드, 사례 카드)가 추가되면 그 영역만 `.container`로 승격한다. 채팅 입력은 어떤 경우에도 `--reading-max`를 유지한다.

---

### 8.8 관리자 차트 다색 팔레트 — 데이터시각화 예외

**규격**: 액센트는 하나다(A-6). 상태 구분에 색을 늘리지 않는다.
**결정**: 관리자 대시보드의 **카테고리 도넛 차트에 한해** `--chart-1`~`--chart-6` 6색을 쓴다.

**근거**: 도넛의 각 조각은 서로 다른 상담 카테고리다. 범주 구분이 차트의 기능 자체이므로 단일 액센트로 축소하면 조각을 구별할 수 없다. §8.1의 흐름도 예외와 같은 논리다 — 단일 액센트 규칙은 인터페이스 크롬을 겨냥한 것이지 정보 그래픽을 겨냥한 것이 아니다.

**제약**

- 차트 색은 `tokens.css`에 정의하고 런타임에 `getComputedStyle`로 읽는다. 하드코딩 금지 — 다크모드 전환이 따라오지 않는다.
- 채도 ≤80%, 배경 대비 ≥6:1 을 만족해야 한다(검증 완료).
- **단일 계열 차트에는 적용하지 않는다.** 일별 추이 막대처럼 계열이 하나면 `--accent` 하나로 충분하며, 실제로 그렇게 구현했다.

### 8.9 내보내기 문서 — 리터럴 값 유지

**규격**: 색·간격·서체 값은 `tokens.css`만 정의하고 페이지는 토큰을 참조한다.
**결정**: PDF·이메일 내보내기가 생성하는 문서의 스타일시트는 **리터럴 값을 그대로 쓴다.**

**근거**: 두 가지가 토큰 참조를 막는다.

1. **다크모드가 인쇄물을 망친다.** 내보내기는 `window.open()`으로 만든 별도 문서다. 여기에 `tokens.css`를 링크하면 `prefers-color-scheme: dark` 블록이 함께 적용돼, OS가 다크 모드인 사용자의 **PDF가 검은 배경으로 출력된다.** 인쇄물은 항상 밝은 배경이어야 한다.
2. **이메일 클라이언트는 외부 스타일시트를 제거한다.** `<link>`는 대부분의 클라이언트에서 살아남지 못하므로 토큰이 정의되지 않은 채 `var()`만 남아 스타일이 통째로 사라진다.

**제약**: 리터럴 값은 라이트 팔레트와 **일치해야 한다.** 팔레트를 바꾸면 이 스타일시트도 함께 고친다 — 자동으로 따라오지 않는다.

| 용도 | 값 | 대응 토큰 |
|------|-----|-----------|
| 본문 | `#111111` | `--color-text` |
| 면·짝수 행 | `#F5F5F5` | `--color-surface` |
| 합계 행 | `#EAEAEA` | (내보내기 전용 — 짝수 행과 구분) |
| 테두리 | `#D4D4D4` | `--color-border` |
| 보조 텍스트 | `#666666` | `--color-text-muted` |
| 표 헤더 | `#111111` 면 + `#FFFFFF` 글자 | `--color-text` / `--color-bg` |

### 8.10 semibold(600) 굵기 추가

**규격**: §3.2.2의 타이포 스케일은 굵기 400·500·700만 지정한다.
**결정**: `--weight-semibold`(600)를 추가한다.

**근거**: 기존 페이지가 600을 실제로 쓰고 있었고, 이를 700으로 올리면 강조가 과해지고 500으로 내리면 위계가 사라진다. §8.3의 h3 추가와 같은 성격 — 규격의 단계 수가 실제 UI의 위계 깊이보다 얕은 경우다.

**제약**: 제목에는 쓰지 않는다(제목은 700). 본문 내 강조와 버튼 레이블에 한정한다.
