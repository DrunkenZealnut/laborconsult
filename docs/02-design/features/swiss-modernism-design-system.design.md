...
| D-3 | `body max-width` | **이중 컨테이너** | 읽기 영역 800px(`--reading-max`), 구조 영역 1280px(`--container-max`). 채팅 답변은 행 길이가 가독성을 좌우하므로 1280px로 넓히지 않는다. **구조 영역의 적용 대상은 §8.6에서 관리자 대시보드로 한정했다.** |
...
**토큰은 이미 정의돼 있다.** `tokens.css`의 `--chart-1`~`--chart-6`이 라이트/다크 값을 갖고 있으며, 관리자 대시보드의 카테고리 도넛이 이미 이 토큰을 쓴다(§8.8). 흐름도 적용 시 새로 정의할 필요 없이 SVG 클래스만 토큰 참조로 바꾸면 된다.
...
4. `line-height`·`font-weight`처럼 토큰이 없어 리터럴이 불가피한 값은 먼저 §3.2에 토큰을 추가한 뒤 치환한다.
