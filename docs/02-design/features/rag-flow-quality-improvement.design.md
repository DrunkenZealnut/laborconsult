...
def _tokenize_ko(text: str) -> list[str]:
    """한국어 간이 토크나이저 — 공백 + 조사 제거.

    Full KoNLPy/mecab 대신 경량 정규식 기반.
    Vercel serverless 환경에서 외부 형태소 분석기 불필요.
    """
    import re
    # 한글 조사/어미 패턴 제거 (간이)
    ...
    return [t for t in tokens if len(t) >= 2]  # 1글자 토큰 제거
...
if params.get("self_rag") and pinecone_hits and len(pinecone_hits) > 2:
    ...
    pinecone_hits = filter_by_relevance(
