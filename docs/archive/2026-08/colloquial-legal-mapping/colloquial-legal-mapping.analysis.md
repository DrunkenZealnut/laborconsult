...
**최종 측정** (Act + CodeRabbit 대응 후): [1] LLM 변환 18/18 · [2] 사전 18/18 · [3] corpus 도달 — 폴백 50%(0%→, SIMPLE 파라미터 정합), 정상 67~83% 반복 4회 준결정 밴드(50%→) · [3'] evidence(승격 3종) 도달 — 폴백 28%, 정상 50%(신규 분리 지표) · `eval_retrieval` 75.0% 무회귀 · 오프라인 스위트 5종 전체 통과(T24 24케이스).
