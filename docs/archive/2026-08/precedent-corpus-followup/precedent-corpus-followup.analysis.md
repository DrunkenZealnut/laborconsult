...
전건 정정 완료**, 관찰 1건(G6)도 즉시 이식. Match Rate 95% ≥ 90% — **Act(자동
반복 수정) 불필요**는 갭 수정에 한정한 판단이다.

**잔여 작업 (사이클 완료 조건):**
1. 커밋·PR·머지 — BM25 gz 포함(미커밋 시 프로덕션 조용한 Dense-only 폴백)
2. **머지 후 프로덕션 검증** — 유실군 판례 질의가 실서비스 `sources` 이벤트에
   잡히는지 확인(선행 사이클 E2E 관례). 이것까지 통과해야 완료 기준 2번
   충족이다.

CodeRabbit 리뷰(7건) 반영 내역은 PR #44 코멘트 참조 — Critical 1건
(pinecone SDK 하한과 `Index.list()` 가용성)은 requirements 하한 상향으로,
Major 2건(hex 절단 충돌·업로드 미확인 삭제)은 전체 hex + 대체 벡터 라이브
확인 게이트로 해소.
