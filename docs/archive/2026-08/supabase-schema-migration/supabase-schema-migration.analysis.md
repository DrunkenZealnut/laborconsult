...
> **Summary**: 설계 대조 103항목 중 98 일치 — **Match Rate 95.1%**. 갭 5건 중 **4건 해소**(G-1·G-2·G-5 검증 완료, G-3 구현 보완). 검증 과정에서 `maybe_single()` 결함을 발견해 5곳 수정했다 — **사용자 글 상세가 한 번도 열린 적이 없었다.** 별도로 Match Rate가 못 잡는 영역을 6항목 실측했다.
>
...
#### G-1. §7 E3 — 게시판 글쓰기·삭제 종단 미검증 → ✅ **해소 (프리뷰·프로덕션 양쪽)**
...
- **조치 결과**: 프리뷰(`f37a5d7`)·프로덕션 양쪽에서 ①CAPTCHA 200 → ②등록 201 → ③recent·search·categories 노출(`source=user`) → ④상세 200 → ⑤오답 403·정답 200 → ⑥소멸+404 전 구간 통과. 검증글은 soft delete 로 정리.
- **부수 성과**: 1차 실행에서 ④가 **500**이었다. `postgrest-py` 의 `.maybe_single().execute()` 가 0행일 때 응답 객체가 아니라 `None` 을 반환해 `.data` 접근이 `AttributeError` → 500 이 된다. `board_detail` 은 `qa_conversations` 를 먼저 조회하는데 게시글 id 로는 항상 0행이라 **사용자 글 상세는 한 번도 열린 적이 없었다.** 이 사이클이 만든 버그가 아니라 `board_posts` 에 행이 생기기 전까지 도달 불가였던 기존 결함이다. 같은 함정이 5곳(`api/index.py` 4 + `legal_api.py` 1)에 있어 `_single_row()` 헬퍼로 전수 봉인했다(`f37a5d7`). **종단 검증을 생략했다면 머지 후 사용자가 처음 발견했을 것이다.**

#### G-2. §9 8단계 — Vercel 환경변수·배포·프로덕션 재검증 → ✅ **해소**
...
- **조치 결과**: Vercel 환경변수 갱신 → PR #49 머지(`a7ff6b0`) → 프로덕션 배포 → 종단 재검증 통과.
- **판별 방법**: 빈 목록 응답(`{"items":[],"total":0}`)만으로는 **"연결 정상 + 데이터 0건"과 "Supabase 미연결 폴백"을 구분할 수 없다** — 이 사이클 내내 문제였던 조용한 실패와 같은 형태다. 실제 글 1건을 등록·삭제해 양성 신호를 얻었다. 신규 코드 반영 자체는 `maybe_single` 수정(없는 id 상세가 500→404)으로 부작용 없이 판별했다.
...
| **갭 5건 중 4건 해소** | G-1·G-2·G-5 검증 완료, G-3은 구현이 설계를 보완 |
| **종단 검증이 값을 했다** | G-1에서 `maybe_single()` 결함 5곳 발견 — 사용자 글 상세가 한 번도 열린 적 없었다 |

후속 이월: **G-4**(쿼터 HTTP 배선 — 이번 사이클에서 변경되지 않은 구간) · §7.1 `SUPABASE_SERVICE_ROLE_KEY` · §7.2 옛 프로젝트 데이터 방침.
