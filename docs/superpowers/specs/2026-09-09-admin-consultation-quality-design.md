...
테이블 `laborconsult.consultation_eval_runs`:
...
(`laborconsult`)를 명시해 평가 테이블 접근도 운영 스키마 안에서 고정한다.
이 스키마 경계는 오프라인 테스트로 게시자·API·SQL 계약이 같은
`laborconsult` 대상을 사용하는지 검증한다. 관리자 JWT는 Supabase에 전달하지 않으며,
