...
- **`output_판례_보강/`는 크롤러가 아니라 법제처 Open API로 채운다** (`fetch_court_precedents.py`). 사건번호 목록만 있으면 원문을 받아올 수 있는 유일한 경로다 — nodong.kr 크롤러들은 게시판 순회 방식이라 특정 사건을 지목할 수 없다. 설계·실측 근거는 `docs/02-design/features/precedent-corpus-expansion.design.md`. 주의점 셋(전부 **조용히** 실패한다):
  ...
  - **법제처 수집 성공률은 단일 기준선으로 예측할 수 없다 — 대상의 사건부호 구성이 지배한다.** 표본으로 잡으면 낙관 편향이 크고(40건 표본 95% → 전량 83%, precedent-corpus-expansion), 교재 인용 판례는 하급심·구판례 비중이 높아 72%였으며(textbook-corpus-embedding, 166건 중 120건), **실무 지침서에서는 21%까지 떨어진다**(yeoncha 47건 중 10건, 2026-09-05). 원인은 스크립트가 아니라 구성이다 — 실측 계열별 수집률은 **하급심 부호(구합·나·누·가단·가합) 2/29 = 7% vs 대법원형(다·두·도) 8/18 = 44%**이고, yeoncha는 하급심 인용이 62%(29/47)였다. 법제처는 하급심을 거의 수록하지 않는다. **수집 전에 대상 CSV의 사건부호 분포를 먼저 세면** 기대치를 맞출 수 있고, 낮은 수집률을 스크립트 결함으로 오진하지 않는다(확인법: 실패 사건번호를 `lawSearch.do`에 직접 넣어 `totalCnt=0`이면 DB 미수록이 확정된다). 미달분은 재시도해도 회수되지 않고 `_미발견.csv`에 누적된다(**2026-09-05 실측 1,793건** — 여러 사이클 누적치). **이 수치는 인용하지 말고 매번 세어 볼 것** — 사이클마다 늘어나는데 문서가 따라가지 못해 156 → 967 → 1,793으로 세 번 어긋났고, 마지막 어긋남은 바로 이 줄을 편집하면서 옆의 숫자를 안 고쳐 생겼다. 세는 법: `_미발견.csv`의 고유 사건번호 수(= `_progress.json`의 `not_found` 크기).
...
- **해설서 헤딩 위생 처리는 "폐기만 범용, 복원은 명시 치환"이다**(`sanitize_heading`). 오폐기는 섹션 경계 하나를 잃을 뿐이지만 오복원은 코퍼스에 오정보를 남긴다. **복원 허용 기준은 "잔해에 표제가 남아 있는가"가 아니라 "복원 내용이 그 문서로 결정되는가 + 치환에 효과가 있는가" 둘 다다.** 금지되는 것은 바깥 지식으로 빈칸을 채우는 추측이지 문서의 다른 부분을 근거로 삼는 일이 아니다 — 실제로 두 판정이 갈린 실례가 있다: `yeoncha`의 `제상.`은 그 위치의 속표지 본문이 장 구성을 나열해 '제1장 서 론'으로 **확정**되므로 복원했고, `gaebyeol`의 `실무데마 19.`는 번호만 남아 제목이 확정되지 않는 데다(목차에서 가져오면 빈칸 채우기) 치환해도 산출물이 바이트 동일해 **효과도 없으므로** 복원하지 않았다. 규칙 수정 시 주의점 셋 — (1) 의미문자 비율의 **분모에서 공백·구두점을 빼야** 한다(`2. 요 건`이 2/6=0.33로 오폐기됨), (2) 조문 표기 추출은 **길이 검사보다 먼저** 해야 한다(59자짜리가 상한 60을 통과해 잡음째 살아남음), (3) `ocr_fixes`는 **원문 키**로 매칭한다(정제 후 매칭하면 정제 로직 변경 시 치환 키가 조용히 무효). 폐기율 10% 초과 시 업로드가 중단된다. 회귀는 T18.
...
- **승인 게이트는 코퍼스가 받쳐주지 않으면 만족 불가능하다.** `official_evidence()`는 Pinecone
  **메타데이터**의 `official_url` 이 공식 호스트 9곳의 https 주소일 것을 요구한다 — 본문에 주소가
  ...
  - **고시는 소관부처 게시판을 먼저 본다**(`fetch_moel_instruction`). law.go.kr 은 행정규칙 페이지를
    **시행일에야** 열어, 발령만 된 고시를 통째로 놓친다 — 2027년 최저임금 고시(제2026–60호)가
    2026-08-05 발령인데 2026-09 기준 여전히 오류페이지였고, 그 탓에 "시행 전이라 수집 불가"로
    넉 달을 미뤄 뒀다. 실제로는 고용노동부 게시판에 발령 당일부터 PDF가 올라와 있었다.
    `moel.go.kr` 은 `OFFICIAL_HOSTS` 라 게시판 주소가 그대로 승인 근거가 된다.
    본문은 게시판에서 받되 **official_url 은 law.go.kr 이 열리면 그쪽을 쓴다** — 정식 인용이고
    bbs_seq 보다 안정적이며, 이미 수집한 고시의 URL 이 바뀌면 sha256 이 달라져 승인 근거가
    무효가 되기 때문이다. 게시판 검색 파라미터(query·searchWord)는 **동작하지 않는다**(기본 목록을
    돌려준다) — `pageUnit=100` 으로 목록을 훑어 제목 정확일치로 찾는다. 첨부는 PDF·HWPX 둘 다 나온다.
  - **보건복지부(`mohw.go.kr`)는 정반대다** — 제목 검색(`keyField=TITLE`)이 동작하고, 게시물이 하루
    수십 건이라 목록 훑기로는 못 찾는다(실측: 최근 150건 안에 없음). 제목이 「…」 고시 일부개정
    형태라 **부분일치**가 필요하다. **본문은 게시판에서 받지 않는다**(`BOARDS[...]["body"] = False`) —
    첨부가 "제1호 가목 중 '400천원'을 '410천원'으로" 같은 일부개정 형식이라 그대로 저장하면 상·하한
    전체 금액이 빠지고, official_url 은 법제처 통합본을 가리켜 저장 본문과 인용 대상이 어긋난다.
    발령번호만 게시판에서 받고 본문은 법제처 통합본에서 받는다.
  - **HWPX 는 정규식으로 태그를 벗기지 말 것.** `</hp:t>` 를 공백으로 바꾸면 서식이 바뀌는 자리에서
    쪼개진 run 사이에 없던 공백이 생긴다("10,700" + "원" → "10,700 원"). 저장 본문이 인용구절 대조의
    기준이라 그 한 칸이 승인을 막는다. XML 파서로 문단(`hp:p`)을 묶고 run(`hp:t`)은 붙여서 잇는다.
  - 고시 금액은 **노동부 게시판이 법제처보다 빠르다**. 출산전후휴가 상한액 고시는 2026-09-17에
    제2026-67호로 개정돼 배우자 유산·사산휴가(3일 252,630원)가 신설됐다 — 금액 갱신 여부는
    게시판 최신 글을 먼저 확인할 것(그 개정에서 기존 금액은 전부 동일했다).
...
- **게이트는 출처만 검증하고 값의 정확성은 검증하지 않는다.** 실측으로 건강보험 **전체** 요율
  0.0719(근로자분은 절반인 0.03595)와 국민연금 조문 본문의 2033년 최종값 0.065(2026년은 부칙의
  0.0475)가 정식 인용까지 붙은 채 승인을 통과했다 — **조문을 정확히 인용할수록 틀린 값이 들어간다**
  (요율 키는 전부 근로자 부담분인데 법령은 전체 요율로 쓰고, 단계 인상은 조문이 아니라 부칙에 있다).
  그래서 관리 화면 입력란 옆의 현재값(`current_parameters()` → `renderCurrent()`)이 **값을 대조하는
  유일한 장치**다. 지울 때 잃는 것이 "표시"가 아니라 "검증"이라는 점에 주의할 것. 경고 임계 30%는
  통상 개정폭(최저임금 2~4%·요율 1~5%)에 걸리지 않으면서 위 두 오입력(×2, ×1.37)을 모두 잡는 값이다.
  내장표 값은 `constants.builtin_parameter()`가 어댑터를 **일부러 우회해** 읽는다 — 계산 경로에서
  쓰면 관리 모드를 무시하게 되므로 표시 전용이다.
- **관리 화면의 공식 원문 목록(`PineconeEvidence.catalog()` → `GET /documents`)은 접두사 나열에
  기댄다.** 메타데이터 필터로는 못 뽑고(벡터 없는 조회 미지원) 임베딩 검색은 관련성 순이라 목록이
  되지 못한다. `_OFFICIAL_PREFIX`가 `pinecone_upload_official_rules.py::ID_PREFIX`와 어긋나면 목록이
  **빈 채로 조용히** 돌아온다. 한 쪽 상한은 **100 미만**이다(실측: `limit=200` → HTTP 400).
  조회 실패는 422로 올리지 말 것 — 문서 ID 직접 입력 경로가 남아 있어야 화면이 멈추지 않는다.
- **인용구절은 저장 원문의 부분문자열이어야 하는데 원문은 PDF 추출물이라 낱말 사이 공백이 없다.**
  웹 화면에서 복사한 정확한 인용일수록 거절됐다(실측: `모든 산업 10,320원` 거절 / `모든산업
  10,320원` 통과). `admin_legal_rules.js::alignQuote()`가 공백을 무시해 위치를 찾은 뒤 **원문 쪽
  문자열로** 교정한다 — 원문을 다듬는 것(적재 규칙 위반)이 아니라 입력을 원문에 맞추는 방향이다.
- **검색은 후보만 만든다.** `sync_legal_rules.py` / 주간 Actions가 Pinecone `laborlaw-v2`를 조회해
  ...
  후보는 `kind="legal_review"`로 고정 생성되므로 **그대로는 영원히 승인할 수 없다**(승인은 `parameter`만
...
- **"지식iN용 복사"(`public/kin_format.js`)는 답변 마크다운을 네이버 지식iN 답변 에디터가 받아주는 인라인 스타일 HTML로 바꿔 리치텍스트로 클립보드에 싣는다**(naver-kin-answer-simplification). 기존 "복사하기"는 마크다운 원문을 그대로 복사해 지식iN에 붙이면 `#`·`**`·`|`가 노출되던 것이 출발점이다. 지킬 것:
  - **허용 태그·속성은 2026-09-22 실측(서식 통과 실험대 https://claude.ai/artifact/23oLDw2W19iHiYrfiGhGAz)에서 통과한 것만이다** — `h2 h3 p b i s span[style] a[href] br hr ul ol li table[style] tr td[style] pre`. `blockquote`·`img`는 깨지고 `class`·`id`·외부 CSS는 전부 버려진다. **새 태그를 쓰려면 실험대 "내 HTML 시험대"에서 먼저 통과시키고 `test_kin_format.js`의 허용 집합을 함께 갱신할 것.** "포맷이 깨졌다"는 보고가 오면 실험대가 첫 진단 수단이다(에디터 규칙은 조용히 바뀐다).
  - **변환은 렌더된 DOM이 아니라 `dataset.md` 원문에서 한다.** `md()`는 class 기반이라 그 출력을 복사하면 서식이 통째로 사라진다. 그래서 파서가 둘이다 — 목적(화면 렌더링 vs 붙여넣기 생존)이 달라 허용 집합이 다르고, `md()`는 `index.html` 인라인이라 node에서 못 부른다.
  - **매핑 규칙 셋은 실측 제약에서 나왔다**: h1→h2·h3 이하→큰 볼드 문단(h1·h4 이하 미실측), 표 머리행은 `th`가 아니라 `td+b`(th 미실측), 인용구는 `<p>`(본문의 `⚠️ **주의사항**:` 라벨이 이미 구분을 담당). `>`는 이스케이프하지 않는다 — 인용 마커 판정에 원문이 필요하고 `md()`도 같은 선택이다.
  - **짝이 안 맞는 마커는 글자로 남긴다.** 절단된 답변(`metadata.truncated`)에서 닫히지 않은 `**`를 삼키면 운영자가 절단을 못 알아챈다.
  - 새 버튼은 **답변만** 모은다 — `getQAPair()`는 `## 질문`을 끼워 넣으므로 쓰지 않는다. 지식iN 답글에 질문 본문이 실리면 안 된다.
  - 복사 경로는 실험대가 검증한 것 그대로(숨김 contenteditable 선택 → `execCommand("copy")` → 폴백 `ClipboardItem`). 지식iN에서 `Ctrl+V`는 서식 유지, `Ctrl+Shift+V`는 텍스트만 — 별도 플레인 변환기는 두지 않는다.
