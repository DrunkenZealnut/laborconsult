...
- **`output_판례_보강/`는 크롤러가 아니라 법제처 Open API로 채운다** (`fetch_court_precedents.py`). 사건번호 목록만 있으면 원문을 받아올 수 있는 유일한 경로다 — nodong.kr 크롤러들은 게시판 순회 방식이라 특정 사건을 지목할 수 없다. 설계·실측 근거는 `docs/02-design/features/precedent-corpus-expansion.design.md`. 주의점 셋(전부 **조용히** 실패한다):
  ...
  - **법제처 수집 성공률은 단일 기준선으로 예측할 수 없다 — 대상의 사건부호 구성이 지배한다.** 표본으로 잡으면 낙관 편향이 크고(40건 표본 95% → 전량 83%, precedent-corpus-expansion), 교재 인용 판례는 하급심·구판례 비중이 높아 72%였으며(textbook-corpus-embedding, 166건 중 120건), **실무 지침서에서는 21%까지 떨어진다**(yeoncha 47건 중 10건, 2026-09-05). 원인은 스크립트가 아니라 구성이다 — 실측 계열별 수집률은 **하급심 부호(구합·나·누·가단·가합) 2/29 = 7% vs 대법원형(다·두·도) 8/18 = 44%**이고, yeoncha는 하급심 인용이 62%(29/47)였다. 법제처는 하급심을 거의 수록하지 않는다. **수집 전에 대상 CSV의 사건부호 분포를 먼저 세면** 기대치를 맞출 수 있고, 낮은 수집률을 스크립트 결함으로 오진하지 않는다(확인법: 실패 사건번호를 `lawSearch.do`에 직접 넣어 `totalCnt=0`이면 DB 미수록이 확정된다). 미달분은 재시도해도 회수되지 않고 `_미발견.csv`에 누적된다(2026-08-30 실측 967건 — 여러 사이클 누적치. 한때 문서들이 156으로 적고 있었다).
