...
- **"지식iN용 복사"(`public/kin_format.js`)는 답변 마크다운을 네이버 지식iN 답변 에디터가 받아주는 인라인 스타일 HTML로 바꿔 리치텍스트로 클립보드에 싣는다**(naver-kin-answer-simplification). 기존 "복사하기"는 마크다운 원문을 그대로 복사해 지식iN에 붙이면 `#`·`**`·`|`가 노출되던 것이 출발점이다. 지킬 것:
  ...
  - **목록 마커 판정은 `md()`와 똑같아야 한다** — 번호는 `\d+\.`, 불릿은 `[-*]`만. 변환기가 `md()`보다 넓게 잡으면(`1)`·`+ `) 화면에 글자로 보이던 것이 지식iN에서 `<ol>`이 되어 에디터가 번호를 다시 매긴다(실기 보고 2026-09-22: 화면 `1)` → 붙여넣기 `1.`). **붙여넣기 결과는 화면과 같아야 한다**는 것이 변환기의 정합 기준이고, 마커 규칙을 한쪽만 바꾸면 그 순간 갈라진다.
