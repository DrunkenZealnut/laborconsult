(function (root) {
  ...
  // 마커 판정은 화면 렌더러 md() 와 **똑같아야** 한다(index.html: 번호 `\d+\.`, 불릿 `[-*]`).
  // 여기서 `1)` 까지 목록으로 잡으면 화면엔 글자 `1)` 로 보이던 것이 지식iN 에서는
  // 에디터가 번호를 다시 매겨 `1.` 로 바뀐다(실기 보고 2026-09-22). `1)`·`+ `·①·• 는
  // md() 처럼 본문 글자로 둔다 — 붙여넣기 결과는 화면과 같아야 한다.
  var BULLET = /^\s*[-*]\s+(.+)$/;
  var NUMBER = /^\s*\d+\.\s+(.+)$/;
