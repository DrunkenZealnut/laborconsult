...
#   토큰 수 변화가 macOS에서는 +36MB로 보였지만 **Linux에서는 사실상 0**이다 —
#   "상한까지 여유가 97MB밖에 안 남았다"는 것은 macOS 한정 서술이었고, 실제
#   프로덕션 여유는 183MB로 변경 전과 같다. 스트리밍 전환 때 macOS 42% vs
#   Linux 3.6%로 갈렸던 것과 정확히 같은 패턴이므로, **이 모듈의 메모리 판단에
#   macOS 수치를 쓰면 매번 과대평가하게 된다.**
#
...
# 색인에서 제외할 상용구·연결어·2음절 문법형·질의 어미(59종).
#
