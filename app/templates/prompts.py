...
14. **판례 검색 키워드 추출** (precedent_keywords — 모든 질문에서 추출):
...
7. **답변 시각화 규칙** (마크다운 적극 활용):
   - **구조**: `##` 소제목으로 섹션을 나누세요 (예: ## 핵심 답변, ## 관련 법조문, ## 관련 판례).
   - **표**: 비교 데이터, 요건 충족 여부, 급여 산정 등은 표로 정리하세요.
   - **강조**: 핵심 결론·금액은 **볼드**, 법조문명은 **볼드**로 표시하세요.
   - **목록**: 절차·조건·요건은 번호 목록(1. 2. 3.)이나 글머리 기호(-)를 사용하세요.
   - **인용문**: 법조문 원문, 판례 요지, 행정해석 회시는 `>` 인용문으로 표시하세요.
