...
ANALYZE_TOOL["input_schema"]["properties"]["precedent_keywords"] = {
    ...
    "description": (
        ...
        "예: '월급이랑 가불을 퉁치재요' → ['임금 상계 금지', '임금 전액 지급 원칙']"
    ),
}
...
16. **판례 검색 키워드 추출** (precedent_keywords — 모든 질문에서 추출):
   ...
   - 사용자의 일상어를 법률 용어로 변환하세요:
     ...
     "다쳤는데 회사가 안 해줘요" → "산재보상"
     "따돌림/괴롭힘/갑질" → "직장 내 괴롭힘"
     "계속 갈군다/갈굼당한다" → "직장 내 괴롭힘", "괴롭힘 신고"
     "일하다 다쳤는데 공상 처리하재요" → "산재보상", "공상 처리"
     "계약직/비정규직" → "기간제 근로자"
     "퉁치자/월급에서 깐다" → "임금 상계 금지", "임금 전액 지급 원칙"
     "밥값/상품권으로 때운다" → "임금 통화 지급 원칙"
     "3.3% 떼는 프리랜서" → "근로자성 인정"
     "4대보험을 안 들어줬다" → "4대보험 가입의무"
