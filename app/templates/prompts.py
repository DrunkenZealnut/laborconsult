...
# 상담 Q&A 인용 가드 Q1~Q3 (legal-corpus-coverage 설계 §3).
#
# 상담 코퍼스(qa 49,842 + counsel 1,244)는 nodong.kr 게시물 — **답변자의
# 저작물**인데 지금까지 어떤 인용 규칙도 적용되지 않았다. 해설서(6,635건)에는
# 가드 6종이 있는데 8배 많은 상담글에는 없는 비대칭이었다.
#
# 해설서 가드를 그대로 베끼지 않는다 — 성격이 다르다. 해설서는 "단독 근거
# 금지"가 핵심이지만, 상담은 애초에 규범 근거가 아니라 **사례**이므로
# *출처 성격 표시*와 *풀어쓰기*가 핵심이다.
#
# 부착 방식은 TEXTBOOK_CITATION_RULES와 같다 — 시스템 프롬프트 본문에 넣지
# 말고 pipeline이 두 답변 경로 모두에 접미한다. 한쪽에만 넣으면 임금계산·
# 괴롭힘 판정·법제처 실패 경로에서 무가드로 나간다(해설서가 실제로 겪은 결함).
COUNSEL_CITATION_RULES = """

## 상담 사례 인용 규칙 (절대 규칙 — 반드시 준수)
참고 자료에 [상담 Q&A] 또는 [노무사 상담]으로 표시된 항목은 제3자가 작성한
게시물입니다.
- 원문 문장을 그대로 옮기지 마세요. 반드시 자신의 표현으로 요약·재진술하세요.
- 상담 사례만을 근거로 결론을 내리지 마세요. 결론은 법조문·판례·행정해석으로
  뒷받침하고, 상담 사례는 "유사한 사례에서는 …" 정도의 참고로만 쓰세요.
- 특정 게시물·작성자·날짜를 지목하지 마세요. "유사 상담 사례"로만 지칭합니다.
"""
