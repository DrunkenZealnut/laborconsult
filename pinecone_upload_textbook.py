...
# marker OCR이 헤딩을 깨뜨린 건들을 명시 치환한다.
#
# **복원 기준은 "잔해에 표제가 남아 있는가"가 아니라 "복원 내용이 이 문서 자체로
# 결정되는가"다.** 금지되는 것은 바깥 지식으로 빈칸을 채우는 일(추측)이지,
# 문서의 다른 부분을 근거로 삼는 일이 아니다. 두 가지가 다 허용된다:
#   · 잔해 복원 — 유효 표제가 남아 있고 잡음만 얹힌 경우(대부분이 여기 해당)
#   · 문맥 결정 — 그 위치의 다른 요소가 표제를 유일하게 확정하는 경우.
#     실례: yeoncha의 `제상.`은 page 20 속표지의 본문이 제1절·제2절을 나열하고
#     그 구성이 목차(page 1)의 '제1장 서 론'과 일치해 다른 답이 없다.
# 반대로 gaebyeol의 `실무데마 19.`는 번호만 남아 제목이 **결정되지 않는다** —
# 목차에서 가져오면 그 위치에 없던 말을 넣는 것이라 추측이다(아래 주석 참조).
#
# 두 번째 기준은 **효과**다. 치환해도 산출물이 바이트 동일하면 하지 않는다 —
# 얻는 것이 없는데 오복원 위험만 진다.
#
...
# '실무테마'→'실무데마' OCR 오독 4건은 치환하지 않는다 — 위 규칙의 **두 기준에
# 모두 걸린다**.
#   · 효과 없음: 모두 직후에 하위 헤딩이 와 본문이 비고, 그래서 섹션에서 탈락해
#     벡터에 나타나지 않는다(치환 유/무 산출물이 바이트 동일함을 확인).
#   · 결정되지 않음: 그중 2건은 번호만 남아('실무데마 19.') 제목이 그 위치의
#     어떤 요소로도 확정되지 않는다. 목차에서 가져오는 것은 빈칸 채우기다.
# yeoncha의 `제상.` 복원과 갈리는 지점이 후자다 — 그쪽은 속표지 본문이 장 구성을
# 나열해 표제가 유일하게 확정된다. "목차를 봤는가"가 아니라 "확정되는가"가 기준이다.
#
...
# '제상.'은 제1장 속표지(page 20)의 헤딩이고, 위 규칙의 **문맥 결정** 사례다
# (잔해 복원이 아니다). 그 위치의 속표지 본문이 '제1절 연차휴가의 의의와 연혁 /
# 제2절 연차휴가제도의 적용범위'를 나열하고, 그 구성이 목차(page 1)의
# '제1장 서 론'과 일치해 **다른 답이 없다** — 추측이 아니라 확정이다.
# 효과 기준도 충족한다: 복원하지 않으면 폐기되어 제1장 본문 19쪽이 직전
# 섹션('일러두기')에 흡수되고, 그 청크들의 출처 카드가 그 이름으로 표시된다.
